...
uses
  ...
  uInterfaces, uTeams, uGuild, uPlayer;

type
  TTeamCheck = class(TForm, IChildren)
    mData: TMemo;
    pName: TPanel;
    lName: TLabel;
    eName: TLabel;
...
function TTeamCheck.AcceptForm: Boolean;
begin
  Result := True;
end;

procedure TTeamCheck.AfterShow;
var
  L: TStringList;
  Intf: IMainMenu;
begin
  ...
  if not (TagObject is TFmxObject) then Exit;
  ...
  begin
    ...
    FileName := TFmxObject(TagObject).TagString + '_guild.json';
    eName.Text := TFmxObject(TagObject).TagString;
    ...
    begin
      ...
      try
        Mdl.LoadData(tcGuild, TFmxObject(TagObject).TagString);
      finally
        FreeAndNil(Mdl);
      end;
    end;
...
procedure TTeamCheck.CheckPlayerTeams(Player: TPlayer);
var
  i,j,k: Integer;
  Idx: Integer;
  TmpS: string;
...
procedure TTeamCheck.CheckTeams;
var
  Guild: TGuild;
  L: TStringList;
  i,j,k: Integer;
...
function TTeamCheck.SetCaption: string;
begin
  Result := 'Check Team';
end;

function TTeamCheck.ShowBackButton: Boolean;
begin
  Result := True;
end;

function TTeamCheck.ShowOkButton: Boolean;
begin
  Result := False;
end;
